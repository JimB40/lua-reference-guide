...
function sectionSlugForPath(pathname) {
  const match = pathname.toLowerCase().match(/\/api-reference\/([^/]+)\/?$/);
  return match ? match[1] : null;
}

function applySectionState(item, label, toggle, isActiveSection) {
  item.classList.toggle("md-nav__item--section-active", isActiveSection);
  label.classList.toggle("md-nav__link--active", isActiveSection);
...
function markMigrationNavItem(primaryNav) {
  const topLevelItems = primaryNav.querySelectorAll(":scope > .md-nav__list > .md-nav__item");
  for (const item of topLevelItems) {
    const label = item.querySelector(":scope > a.md-nav__link, :scope > label.md-nav__link");
    const labelText = getNavLabelText(label);
    if (labelText === "Migration" || labelText === "Migration / Update") {
      item.classList.add("md-nav__item--migration");
      const ellipsis = label?.querySelector(".md-ellipsis");
      if (ellipsis && ellipsis.textContent?.trim() === "Migration / Update") {
        ellipsis.textContent = "Migration";
      }
      const nestedTitle = item.querySelector(":scope > nav.md-nav > label.md-nav__title");
      if (nestedTitle && nestedTitle.textContent?.includes("Migration / Update")) {
        nestedTitle.innerHTML = nestedTitle.innerHTML.replaceAll("Migration / Update", "Migration");
      }
    } else {
      item.classList.remove("md-nav__item--migration");
    }
  }
}
...
function buildBreadcrumbText(text) {
  const span = document.createElement("span");
  span.className = "api-function-breadcrumb__current";
...
function renderApiFunctionBreadcrumb(sectionLink) {
  ...
  const sectionHref = new URL(sectionLink.getAttribute("href") || sectionLink.href, window.location.origin).pathname;
  ...
  appendBreadcrumbSeparator(breadcrumb);
  breadcrumb.appendChild(buildBreadcrumbText(heading.textContent?.trim() || ""));

  article.insertBefore(breadcrumb, heading);
}

function highlightApiReferenceFunctionGroup(primaryNav) {
  const activeSectionKey = sectionKeyForPath(window.location.pathname);
  const activeSlug = sectionSlugForPath(window.location.pathname);
  if (!activeSectionKey || !activeSlug) {
    clearApiFunctionBreadcrumb();
    return;
  }

  const apiReferenceItem = Array.from(primaryNav.querySelectorAll(":scope > .md-nav__list > .md-nav__item--nested"))
    .find((item) => getNavLabelText(item.querySelector(":scope > label.md-nav__link")) === "API Reference");
  if (!apiReferenceItem) {
    clearApiFunctionBreadcrumb();
    return;
  }
  ...
  const sectionLinks = apiReferenceItem.querySelectorAll(":scope > nav.md-nav > .md-nav__list > .md-nav__item > a.md-nav__link");
  const matchingSectionLink = Array.from(sectionLinks).find((link) => getNavLabelText(link) === activeSectionKey);
...
async function mountNavSectionLinks() {
  ...
  for (const item of nestedItems) {
    ...
    const isActiveSection = Boolean(activeSectionKey && parentText === activeSectionKey);
    ...
    applySectionState(item, label, toggle, isActiveSection);

    if (label.dataset.sectionLinkBound === "true") continue;
    label.dataset.sectionLinkBound = "true";
  ...
  highlightApiReferenceFunctionGroup(primaryNav);
}

function setupNavSectionLinks() {
  void mountNavSectionLinks();
}

if (typeof document$ !== "undefined") {
  document$.subscribe(setupNavSectionLinks);
} else if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", setupNavSectionLinks, { once: true });
} else {
  setupNavSectionLinks();
}
