...
`zone` and `options` are stored in the [closure](../api-overview/lua-basics.md#closures) of `loadable.lua`, therefore they do not need to be added to the `widget` table, as is commonly done.
