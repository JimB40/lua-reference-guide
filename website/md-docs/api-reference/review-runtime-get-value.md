...
<tr><td><code>-</code></td><td>return</td><td><code>integer</code></td><td>current source value (number). Zero is returned for: * non-existing sources
...
<tr><td><code>-</code></td><td>return</td><td><code>table</code></td><td>GPS position is returned in a table: * `lat` (number) latitude, positive is North
...
@retval (integer) current source value (number). Zero is returned for: * non-existing sources
 * for all telemetry source when the telemetry stream is not received
 * far all non allowed sensors while FAI MODE is active

@retval (table) GPS position is returned in a table: * `lat` (number) latitude, positive is North
 * `lon` (number) longitude, positive is East
 * `pilot-lat` (number) pilot latitude, positive is North
 * `pilot-lon` (number) pilot longitude, positive is East
