...
This normal key event sequence can be altered with the [killEvents(key)](../../api-reference/key-inputs/kill-events.md) function. Any time this function is called (after the `FIRST` event) all further key events for this key will be suppressed until the next key press of this key. Examples:
