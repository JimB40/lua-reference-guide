# Review: getRotEncMode

[Back to dashboard](review.md) | [Open API page](runtime-get-rot-enc-mode.md)

## Snapshot

- Module: `runtime`
- Current symbol: `getRotEncMode`
- Doc id: `runtime.get-rot-enc-mode`
- Source: `radio/src/lua/api_general.cpp:874`
- Recommended action: **No action needed**
- Why: Parsed output looks structurally healthy.

## Decide

- This one looks structurally complete, but you can still change any param or return type below if you want to override it.

### Decision Controls

<div class="decision-panel" data-item-id="getRotEncMode">
<div class="decision-status" data-role="status">Decision server not connected yet.</div>
<table class="decision-table">
<thead><tr><th>Field</th><th>Kind</th><th>Current Type</th><th>Description</th><th>Select Type</th></tr></thead>
<tbody>
<tr><td><code>-</code></td><td>return</td><td><code>integer</code></td><td>in list: Normal = 0, Both V and H inverted = 1, V-N = 2, V-A = 3 return 0 on radio without rotary encoder</td><td><select data-field-kind="return" data-field-name="-" data-current-type="integer"><option value="TODO">TODO</option><option value="integer">integer</option><option value="number">number</option><option value="string">string</option><option value="boolean">boolean</option><option value="table">table</option><option value="function">function</option><option value="pointer">pointer</option><option value="nil">nil</option><option value="integer|string">integer|string</option><option value="table|nil">table|nil</option><option value="function|nil">function|nil</option><option value="string|nil">string|nil</option></select><input type="text" placeholder="Custom type" data-field-kind="return" data-field-name="-" data-custom-type="true" /></td></tr>
</tbody>
</table>
<div class="decision-actions"><button type="button" data-role="save">Save Decisions</button></div>
<script type="application/json" class="decision-payload">{&quot;item_id&quot;: &quot;getRotEncMode&quot;, &quot;symbol&quot;: &quot;getRotEncMode&quot;, &quot;summary&quot;: &quot;Return rotary encoder mode&quot;, &quot;parameters&quot;: [], &quot;returns&quot;: [{&quot;name&quot;: &quot;&quot;, &quot;type&quot;: &quot;integer&quot;, &quot;description&quot;: &quot;in list: Normal = 0, Both V and H inverted = 1, V-N = 2, V-A = 3 return 0 on radio without rotary encoder&quot;}], &quot;notes&quot;: [], &quot;since&quot;: &quot;2.8.0&quot;}</script>
</div>

<div class="review-workbench">
<section class="review-panel">
### Source luadoc

~~~~text
@function getRotEncMode()

Return rotary encoder mode

@retval number in list: Normal = 0, Both V and H inverted = 1, V-N = 2, V-A = 3
  return 0 on radio without rotary encoder

@status current Introduced in 2.8.0
~~~~
</section>
<section class="review-panel">
### Parsed doc

#### Summary

Return rotary encoder mode

#### Parameters

None.

#### Returns

| Name | Type | Description |
| --- | --- | --- |
| `-` | `integer` | in list: Normal = 0, Both V and H inverted = 1, V-N = 2, V-A = 3 return 0 on radio without rotary encoder |

#### Notes

None.

</section>
</div>

## Suggested luadoc patch

Use this as a starting point when the issue is in the C++ annotation. `TODO` means you still need to choose the real type.

~~~~text
@function getRotEncMode()

Return rotary encoder mode

@retval (integer) in list: Normal = 0, Both V and H inverted = 1, V-N = 2, V-A = 3 return 0 on radio without rotary encoder

@status current Introduced in 2.8.0
~~~~
